...
pub(crate) mod shader_resources {
    ...
    pub(super) fn build_shader_blob(entry: ShaderModule, target: ShaderTarget) -> Result<ID3DBlob> {
        unsafe {
            ...
            // zdb patch: prefer shaders bundled next to the executable so the
            // binary is self-contained (the original reads from the build-time
            // CARGO_MANIFEST_DIR, which does not exist on an end user's machine).
            let bundled = std::env::current_exe()
                .ok()
                .and_then(|p| p.parent().map(|d| d.join("shaders").join(shader_name)))
                .filter(|p| p.exists());
            let shader_path = match bundled {
                Some(p) => p,
